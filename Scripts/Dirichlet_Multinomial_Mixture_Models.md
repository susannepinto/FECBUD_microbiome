...
![](Dirichlet_Multinomial_Mixture_Models_files/figure-gfm/Plot%20importance-1.png)<!-- -->
...
bars <- ggplot( df.bar.resp, aes( fill = cluster, y = n, x = clinical_outcome_wk14 )) + geom_bar( position = "fill", stat = "identity", width = 0.7 ) + ylab( "Percentage (%)" ) + theme( panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
``` r
plot.clusters = ( clus  + theme(legend.position = "none") | bars ) / pca.1.3 + plot_layout( nrow = 2 ) + plot_layout(guides = "collect")
plot.clusters
```
