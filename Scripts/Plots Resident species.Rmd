...
load("/Volumes/My Passport for Mac/FECBUD studie/FECBUD GitHub/Data/Scenario2_list.Rda")
```
...
final_df$abundance.difference.trans <- sign(final_df$abundance.difference) * log(abs(final_df$abundance.difference))
...
# model
library(lme4)
lmer(abundance.difference ~ State + ( 1 | subject_id ), data = final_df)
```
...
final_df$abundance.PreFMT.trans <- sign(final_df$abundance.PreFMT) * log(abs(final_df$abundance.PreFMT))
